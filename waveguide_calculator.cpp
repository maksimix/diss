...
void applyAccuracyLevel(em::FemSolverSettings &fem, int level)
{
    ...
    case 0:   // быстро
        ...
        // Отношение 2, а не 8. Глобальная ячейка на этом уровне вдвое мельче,
        // чем на «обычном» (элементы первого порядка требуют более частой
        // сетки), поэтому прежнее отношение 8 давало самый мелкий пол
        // измельчения из всех трёх уровней: 0.26 мм против 1.04 мм у
        // «обычного». Уровень с названием «быстро» получался самым тяжёлым —
        // на круглом тракте с Т-вставками он просил 184 тыс. неизвестных и
        // 33.5 ГБ там, где «обычное» обходилось 61 тыс. и 5.9 ГБ. При
        // отношении 2 пол измельчения совпадает с «обычным» в абсолютных
        // миллиметрах, а контраст сетки падает до двух-трёх к одному, где
        // итерационный решатель уверенно сходится и память не нужна вовсе.
        fem.mesh.minimum_size_ratio = 2.0;
