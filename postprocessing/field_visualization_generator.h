...
struct FieldVisualizationSettings
{
    ...
    // Концентрация линий поля: во сколько раз больше или меньше силовых линий
    // строится по сравнению с обычной плотностью. Значение зажимается в
    // [0.25, 4.0] и умножает число точек посева — объёмных для линий E и H и
    // пристеночных для линий тока. Сама трассировка не меняется: каждая линия
    // остаётся полной интегральной кривой поля, гуще становится только их сеть.
    //
    // У доминирующей TE10 электрическое поле показано не линиями, а амплитудной
    // эпюрой: каждая её стрелка — трубка потока, взятая по равным квантилям, то
    // есть ровно та же «линия поля», только нарисованная иначе. Поэтому эпюра
    // подчиняется этой же настройке.
    //
    // Глифы направления — стрелки H, J и потока Пойнтинга — ей не подчиняются:
